...
const ImagesGallery = ({ images }) => {
   ...
   const handleNextImage = () =>
      setActualImagePosition((actualImagePosition) => actualImagePosition + 1);
            ...
            >
               {images &&
                  images.map((image, index) => (
                     <li key={index}>
                        <img src={image} alt={`Product model ${index}}`} />
                     </li>
                  ))}
            </ul>
         </div>
         <ul className='images-preview'>
            {images &&
               images.map((image, index) => (
                  <li
                     key={index}
                     className={index === actualImagePosition ? 'selected' : ''}
                  >
                     <img
                        src={image}
                        alt={`Product model ${index}}`}
                        onClick={() => handleChangeImagenPosition(index)}
                     />
                  </li>
               ))}
         </ul>
